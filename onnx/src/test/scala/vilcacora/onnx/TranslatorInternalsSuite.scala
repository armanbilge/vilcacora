...
class TranslatorInternalsSuite extends FunSuite {
  ...
  test("parseShape should fail on a dynamic dimension parameter") {
    val shapeProto = TensorShapeProto(
      dim = Seq(TensorShapeProto.Dimension(value = TensorShapeProto.Dimension.Value.DimParam("unk_0")))
    )
  ...
  test("parseShape should fail on an empty (unknown) dimension") {
    val shapeProto =
      TensorShapeProto(dim = Seq(TensorShapeProto.Dimension(value = TensorShapeProto.Dimension.Value.Empty)))
    val result = Translator.parseShape(shapeProto)
  ...
  test("createAllocation should build a valid Allocation from a ValueInfoProto") {
    val shape = TensorShapeProto(dim = Seq(TensorShapeProto.Dimension(TensorShapeProto.Dimension.Value.DimValue(10))))
    val typeProto = TypeProto(value = TypeProto.Value.TensorType(TypeProto.Tensor(elemType = 1, shape = Some(shape))))
  ...
  test("extractBytes should fall back to typed data fields") {
    ...
    val expectedBytes = ByteBuffer.allocate(8).order(ByteOrder.LITTLE_ENDIAN).putFloat(1.0f).putFloat(-1.0f).array()
