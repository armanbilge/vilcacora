...
class TranslatorSuite extends FunSuite {
  ...
  test("translateNode should translate a Cast node") {
    val node = NodeProto(
      ...
      input = Seq("in"),
      output = Seq("out"),
      attribute = Seq(AttributeProto(name = "to", i = 6, `type` = AttributeProto.AttributeType.INT)) // 6 = INT64
    )
    assertEquals(Translator.translateNode(node), Right(Operation.Cast("in", "out", DataType.Int64)))
  }

  // --- REWRITTEN TEST FOR SVMCLASSIFIER ---
  test("translateNode should correctly translate an SVMClassifier node with floating point tolerance") {
    val node = NodeProto(
